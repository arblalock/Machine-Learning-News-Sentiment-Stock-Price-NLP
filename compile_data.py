...
news_df["sentiment_description"] = model.predict(
    np.asarray(news_df["description"].astype(str))
)
...
dates = list(
    set(
        map(
            lambda x: datetime.strptime(x.split(" ")[0], "%m/%d/%Y").strftime(
                "%Y-%m-%d"
            ),
            list(stock_df["date"]),
        )
...
formatted_stock_df["open_price"] = list(
    stock_df[stock_df["date"].str.contains(" 9:30")]["4. close"]
)
formatted_stock_df["noon_price"] = list(
    stock_df[stock_df["date"].str.contains("12:00")]["4. close"]
)
formatted_stock_df["330_price"] = list(
    stock_df[stock_df["date"].str.contains("15:30")]["4. close"]
)
formatted_stock_df["close_price"] = list(
    stock_df[stock_df["date"].str.contains("16:00")]["4. close"]
)

formatted_stock_df["LG_12-330"] = np.where(
    formatted_stock_df["noon_price"] > formatted_stock_df["330_price"], 0, 1
)
formatted_stock_df["LG_12-close"] = np.where(
    formatted_stock_df["noon_price"] > formatted_stock_df["close_price"], 0, 1
)
...
news_df["date"] = news_df["date"].apply(
    lambda x: datetime.strptime(x, "%m/%d/%Y").strftime("%Y-%m-%d")
)


training_df = formatted_stock_df.copy()
training_df["headline_mean"] = formatted_stock_df["date"].apply(
    lambda x: news_df[news_df["date"] == x]["sentiment_headline"].mean()
)
training_df["description_mean"] = formatted_stock_df["date"].apply(
    lambda x: news_df[news_df["date"] == x]["sentiment_description"].mean()
)
training_df["content_mean"] = formatted_stock_df["date"].apply(
    lambda x: news_df[news_df["date"] == x]["sentiment_content"].mean()
)
