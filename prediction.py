...
for x in range(len(X_test)):
    if y_pred_reg[x] >=  train_df['noon_price'][X_test.index[x]] and train_df['LG_12-330'][X_test.index[x]] == 1:
        regression_df['reg_correct'][X_test.index[x]] = 1
    elif y_pred_reg[x] <  train_df['noon_price'][X_test.index[x]] and train_df['LG_12-330'][X_test.index[x]] == 0:
        regression_df['reg_correct'][X_test.index[x]] = 1
    else:
...
# %% 
#Classification
class_model = RandomForestClassifier(n_estimators = 10, criterion = 'entropy', random_state = 0)
